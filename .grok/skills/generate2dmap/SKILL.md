...
| Image tools | `imagine_text_to_image` / `imagine_image_to_image` (path-based; see **`imagine`**) |
| Inspect images | `read_file` on PNG paths (not Codex view_image) |
| Generated image path | `imagine_text_to_image` → sandbox `file_path` → copy into `assets/map/` |
...
2. Pass that `file_path` on the next `imagine_image_to_image` call — this is what actually hands the reference to the image model. Also call `read_file` on the saved file so you can see it yourself.
3. In the next `imagine_image_to_image` prompt, explicitly say to use the provided reference image as the visual reference.
...
Do not rely on a filename, prompt wording like "based on the map", or the image merely being visible in conversation as the reference handoff. The base/background must be passed as the tool's source `file_path` argument; if it is not, stop and pass it before generating the dressed reference or stage reference.
